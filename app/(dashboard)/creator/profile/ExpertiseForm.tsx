            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-8">
                {categories.map((cat) => (
                    <label key={cat.slug} className={`flex items-center p-4 rounded-xl border-2 cursor-pointer transition-all active:scale-[0.99] touch-manipulation ${savedSpecializations.includes(cat.slug)
                        ...
                        : 'border-[#F0F9FF] hover:border-gray-300'
                        }`}>
                        <input
                            type="checkbox"
                            ...
                            className="w-5 h-5 text-[#0EA5E9] border-gray-300 rounded focus:ring-[#0EA5E9] mr-3"
                        />
                        <div className="flex-1">
                            <span className="font-bold text-base block text-[#1E293B]">{cat.title}</span>
                        </div>
                    </label>
                ))}
            </div>
